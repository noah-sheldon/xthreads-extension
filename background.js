...
class XThreadsBackground {
  ...
  async handleMessage(message, sender, sendResponse) {
    try {
      switch (message.action) {
        ...
        case "generateReply":
          const replyResult = await this.handleGenerateReply(message.tweetData || { content: message.tweetText });
  ...
  async handleGenerateReply(tweetData) {
    try {
      console.log("Background: Generating manual reply for:", tweetData.content?.substring(0, 50));
      
      const settings = await this.getSettings();
      
      if (!settings.apiKey || !settings.selectedBrandId) {
        return { success: false, error: "Missing API key or brand space" };
      }
      ...
      if (!reply) {
        return { success: false, error: "No reply generated by API" };
      }
      ...
      console.log("Manual reply generated:", reply.substring(0, 50));
      return { success: true, reply };

    } catch (error) {
      console.error("Failed to generate manual reply:", error);
